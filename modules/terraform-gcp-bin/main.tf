locals {
  ...
  gcp_instance_images                   = distinct(var.gcp_instance_images)
  ...
  gcp_instance_rules = {
    for rule in var.gcp_instance_clusters : rule["cluster"] => rule
  }
}

data "google_kms_key_ring" "this" {
  for_each = local.gcp_instance_rules
...
resource "google_container_analysis_note" "this" {
  ...
  attestation_authority {
    hint {
      human_readable_name = each.value["cluster"]
    }
  }
}
...
resource "google_binary_authorization_policy" "this" {
  ...
  dynamic "admission_whitelist_patterns" {
    for_each = local.gcp_instance_images
  ...
  dynamic "cluster_admission_rules" {
    ...
    content {
      cluster                 = format("%s.%s", local.gcp_instance_region, cluster_admission_rules.value["cluster"])
      evaluation_mode         = local.gcp_instance_cluster_evaluation_mode
      enforcement_mode        = local.gcp_instance_cluster_enforcement_mode
      require_attestations_by = [google_binary_authorization_attestor.this[cluster_admission_rules.key].name]
    }
  }
}
